...
class PicamSensor(Sensor):
    ...
    def get_reading(self):
        if self.image_gen is None:
            print('initializing camera with resolution: %s'%str(self.shape))
            self.cam = PiCamera()
            # apply user-supplied settings
            for key,val in self.camargs.items():
                setattr(self.cam, key, val)
            self.image_gen = self.cam.capture_continuous(BytesIO(), 'jpeg', resize=self.shape, use_video_port=True)
        #print('getting video')
        #return b'1234567890'
